# 🎮 Designer Playstyle

> **The way you play games reflects the way you design.**
> Discover your design archetype through a playful interactive quiz.

Designer Playstyle is an interactive archetype quiz created for the **Friends of Figma Jakarta • CONFIG Watch Party 2026**.
Instead of asking traditional design-related questions, the quiz explores how people approach games as a metaphor for how they think, collaborate, and solve problems as designers.

---

# ✨ Why?

Every designer has a different way of approaching their craft.
Some naturally think several steps ahead before taking action.
Some thrive on precision and flawless execution.
Others prioritize people, collaboration, and creating meaningful experiences.
Designer Playstyle transforms these tendencies into a lightweight, game-inspired experience that's fun to complete while still offering meaningful insights.

---

# 🎯 The Three Dimensions

Every answer contributes to one of three core dimensions.

| Dimension | Represents                                          |
| --------- | --------------------------------------------------- |
| **HOW**   | Technical execution and craftsmanship               |
| **WHO**   | Empathy, collaboration, and human-centered thinking |
| **WHY**   | Strategy, vision, and long-term decision making     |

These dimensions determine your Designer Playstyle archetype.

---

# 📝 Quiz Structure

* 7 questions
* 3 choices per question
* Each answer contributes points toward one of the following dimensions:

  * **Technic (HOW)**
  * **Empathic (WHO)**
  * **Strategic (WHY)**

The scoring order changes between questions, making the outcome difficult to predict and encouraging authentic answers.

---

# 🧠 Scoring

At the end of the quiz, the system calculates three independent scores:

* HOW (Technic)
* WHO (Empathic)
* WHY (Strategic)

These scores are visualized in a stat chart, and the highest dimension determines the player's primary archetype.

---


# 📊 Results

After completing the quiz, every participant receives:

* Their primary Designer Playstyle archetype
* A HOW • WHO • WHY stat chart
* A personalized archetype description
* A growth recommendation (Next Move)

---

# 🎨 Design Philosophy

Designer Playstyle was designed to feel more like a game than a personality test.
The experience is intentionally:

* Playful
* Fast to complete
* Visually engaging
* Easy to understand
* Shareable

Rather than placing designers into rigid personality categories, the quiz highlights natural tendencies while encouraging growth across all three dimensions.

---

# 🛠 Built With

This project was built **entirely using Figma Make**, from interface design to interactive quiz logic.
The goal was to explore how far Figma Make can go in creating a polished, production-ready interactive experience without relying on a traditional frontend framework.
Features implemented with Figma Make include:

* Interactive UI flows
* Quiz navigation
* State management
* Score calculation
* Dynamic result generation
* Responsive layouts
* Social sharing support with Open Graph images

No React setup. No custom frontend framework. Just Figma Make turning an idea into a fully interactive web experience.

---

# ❤️ Credits

Created for **Friends of Figma Jakarta • CONFIG Watch Party 2026**

Designed and built with **Figma Make**

A collaboration between **Ryan Pratama (Head of Creative Operations)** and **Arya Maulana (Product Designer)**.

Made with ❤️ by **Gizalab**

